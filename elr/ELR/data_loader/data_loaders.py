...
# from data_loader.imagenet200 import get_imagenet200
from data_loader.TIN import get_TIN
...
class TINDataloader(BaseDataLoader):
    def __init__(self, data_dir, batch_size, shuffle=True, validation_split=0.0, num_batches=0, training=True,num_workers=8, pin_memory=True,img_size=64):
        config = ConfigParser.get_instance()
        cfg_trainer = config['trainer']
        ...
        noise_file = cfg_trainer.get('noise_file', None)

        self.train_dataset, self.val_dataset = get_TIN(config['data_loader']['args']['data_dir'], cfg_trainer, train=training,
                                                           transform_train=transform_train, transform_val=transform_val, noise_file = noise_file)
