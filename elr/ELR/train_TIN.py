...
def main(config: ConfigParser):
    wandb.init(
        project="ELR_Noisy_dataset_val",
        name=f"ELR_{config['name']}_{config['trainer']['epochs']}_beta_{config['train_loss']['args']['beta']}_lambda_{config['train_loss']['args']['lambda']}_{config['seed']}_{config['trainer']['percent']}",
        config=config  
    )
    ...
    model = ResNet18_64x64(num_classes=config['num_classes'])
...
if __name__ == '__main__':
    ...
    options = [
        ...
        CustomArgs(['--noise_file'], type=str, target=('trainer', 'noise_file'))
    ]
