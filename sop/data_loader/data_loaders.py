...
class TINDataloader(BaseDataLoader):
    def __init__(self, data_dir, batch_size, shuffle=True, validation_split=0.0,
                 num_batches=0, training=True, num_workers=8, pin_memory=True,img_size=64):

        config = ConfigParser.get_instance()
        cfg_trainer = config['trainer']

        transform_train = transforms.Compose([
        # transforms.RandomResizedCrop(img_size, interpolation=InterpolationMode.BILINEAR,padding=4),
        ...
        transform_val = transforms.Compose([
            # transforms.Resize((64, 64), interpolation=InterpolationMode.BILINEAR),  # 크기 고정
        ...
        if config['data_augmentation']['type'] is not None:
            autoaug = transforms.Compose([])
            if config['data_augmentation']['type'] == 'fa_reduced_tinyimagenet':
                autoaug.transforms.insert(0, Augmentation(fa_reduced_cifar10()))
            elif config['data_augmentation']['type'] == 'autoaug_tinyimagenet':
                autoaug.transforms.insert(0, Augmentation(autoaug_imagenet_policy()))
            elif config['data_augmentation']['type'] == 'autoaug_extend_tinyimagenet':
                autoaug.transforms.insert(0, Augmentation(autoaug_policy()))
            elif config['data_augmentation']['type'] == 'default':
                pass
            else:
        ...
        self.train_dataset, self.val_dataset = get_TIN(
            config['data_loader']['args']['data_dir'], cfg_trainer, training,
            transform_train=transform_train, transform_train_aug=transform_train_aug,
            transform_val=transform_val)

        super().__init__(self.train_dataset, batch_size, shuffle, validation_split,
                         num_workers, pin_memory, val_dataset=self.val_dataset)
