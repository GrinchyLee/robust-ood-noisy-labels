...
if __name__ == "__main__":
    ...
    name=f"{args.dataset}_{str(args.seed)}_{str(args.r)}"  # 실험 이름
    )
    ...
    while epoch < args.num_epochs:
        ...
        if epoch < args.warm_up:
            ...
            if (args.r == 0) and (epoch<=200):# and (args.dataset=='cifar10'):
                pred1 = prob1 > 0.99999
                pred2 = prob2 > 0.99999
            else:
                pred1 = prob1 > 0.5
        ...
        if acc > best_acc:
            best_acc = acc  # 최고 정확도 갱신
            best_model_path = os.path.join(args.checkpoint_path, "saved", str(args.seed),"best_model_info.pth.tar")
            torch.save(data_dict, best_model_path)
            print(f"New best model saved at epoch {epoch} with accuracy: {acc:.3f}%")
