...
if __name__ == "__main__":
    args = parse_args("presets_prop3_seed0.json")
    ...
    wandb.init(
    project="PGDF",
    name=f"{args.dataset}_{str(args.r)}_{str(args.seed)}"  
    )
    ...
    def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, easy_trainloader):
        ...
        for (
            ...
            input_x_nomix = torch.cat([inputs_x, inputs_x2])
            target_x_nomix = torch.cat([targets_x,targets_x])
            
            w_hard = torch.cat([w_x,w_x])
            logits_x = net(input_x_nomix)
            
            
            if unlabeled_trainloader is not None:
                inputs_u_nomix = torch.cat([inputs_u, inputs_u2], dim=0)
                target_u_nomix = torch.cat([targets_u, targets_u], dim=0)
                w_hard = torch.cat([w_x,w_x])
                idx = torch.randperm(inputs_u_nomix.size(0))
                input_a, input_b = inputs_u_nomix, inputs_u_nomix[idx]
                target_a, target_b = target_u_nomix, target_u_nomix[idx]
                mixed_input = l * input_a + (1 - l) * input_b
                mixed_target = l * target_a + (1 - l) * target_b
                logits_u = net(mixed_input)
                logits = torch.cat([logits_x,logits_u])
            else:
                logits_u = None
                logits = logits_x
    ...
    if args.pretrained_path != "":
        with open(args.pretrained_path + f"/saved/{args.seed}/{args.preset}.pth.tar", "rb") as p:
