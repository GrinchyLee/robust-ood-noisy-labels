...
from dataloader_tinyimagenet import Tinyimagenet_dataset
import dataloader_tinyimagenet as dataloader

if __name__ == "__main__":
    args = parse_args("./presets_tinyimagenet.json")
    ...
    def warmup(epoch, net, optimizer, dataloader, recorder):
        ...
        progress_bar = tqdm(enumerate(dataloader), total=num_iter, desc=f"Epoch {epoch+1}")
        for batch_idx, (inputs, labels, path) in progress_bar:
    ...
    with open(f"{args.checkpoint_path}/saved/{args.seed}/recorder1.p","wb") as f1:
    ...
    train_dataset = Tinyimagenet_dataset(dataset=args.dataset,
                            r=args.r,
                            noise_mode="tinyimagenet",
                            root_dir=args.data_path,
                            # noise_file=f"{args.checkpoint_path}/saved/labels.json",
                            noise_file=args.noise_file,
                            transform="",
                            mode="all")
    train_dataset.train_data = np.array(train_dataset.train_data)
    ...
    with open(f"{args.checkpoint_path}/saved/{args.seed}/train_data_easy.p","wb") as f1:
        pickle.dump(train_data_easy,f1)
    with open(f"{args.checkpoint_path}/saved/{args.seed}/train_label_easy.p","wb") as f1:
        pickle.dump(train_label_easy,f1)

    #inject noise to D_e
    easy_loader = dataloader_easy_tinyimagenet.easy_dataloader(
        dataset=args.dataset,
        r=args.r,
        noise_mode=args.noise_mode,
        batch_size=args.batch_size,
    ...
    while epoch < args.num_epochs:
        ...
        with open(f"{args.checkpoint_path}/saved/{args.seed}/recorder_easy.p", "wb") as f:
            pickle.dump(recorder_easy, f)
        ...
        with open(f"{args.checkpoint_path}/saved/{args.seed}/easy_labels.p","rb") as f1:
            noise_label = pickle.load(f1)
        noise_rate=args.r
        epoch += 1
    
    with open(f"{args.checkpoint_path}/saved/{args.seed}/recorder_easy.p","wb") as f1:
    ...
    for index in train_index:
        print(train_label[index],noise_label[index])
    ...
    for epoch in range(max_epoch):
        ...
        for i, (train_data, train_label) in enumerate(tqdm(train_dataloader_ehn, desc=f"Epoch {epoch+1}")):
    ...
    pred = [False for i in range(100000)]
    prob = [0 for i in range(100000)]
    ...
    for ind in sort_index[int(sort_index.shape[0]*(0.5+noise_rate*0.5)):]:
        prob[ind]=1 # easy
    prob = np.array(prob)
    with open(f"{args.checkpoint_path}/saved/{args.seed}/prob1_ehn.p","wb") as f:
        pickle.dump(prob,f)
    ...
    for ind in sort_index[int(sort_index.shape[0]*(0.5+noise_rate*0.5)):]:
        prob[ind]=1 # easy
    prob = np.array(prob)
    with open(f"{args.checkpoint_path}/saved/{args.seed}/prob2_ehn.p","wb") as f:
        pickle.dump(prob,f)