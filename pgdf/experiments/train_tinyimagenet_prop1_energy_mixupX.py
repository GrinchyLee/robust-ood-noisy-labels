...
if __name__ == "__main__":
    ...
    def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, easy_trainloader):
        ...
        if epoch > 150:
            ...
            #추가
            # eye_mat = torch.eye(temp_prob_trans_m.size(0), device=temp_prob_trans_m.device)
            # temp_prob_trans_m = temp_prob_trans_m + 1e-8 * eye_mat
            ########################################################
        ...
        for (
            ...
            w_x_flatten = w_x.view(-1)
            # print("w_x:",w_x.shape)
            easy_mask = (w_x_flatten == 1)
            hard_mask = (w_x_flatten < 1.0)
            # print(len(targets_x))
            inputs_easy1 = inputs_x[easy_mask]
            inputs_easy2 = inputs_x2[easy_mask]
            targets_easy = targets_x[easy_mask]
            # print(len(targets_easy))
            inputs_hard1 = inputs_x[hard_mask]
            inputs_hard2 = inputs_x2[hard_mask]
            targets_hard = targets_x[hard_mask]
            # print(len(targets_hard))


            # easy set 처리
            has_easy = inputs_easy1.size(0) > 0
            # hard set 처리
            has_hard = inputs_hard1.size(0) > 0
            
            w_hard = torch.cat([w_x,w_x])
            if has_easy:
                input_easy_nomix = torch.cat([inputs_easy1, inputs_easy2], dim=0)
                target_easy_nomix = torch.cat([targets_easy, targets_easy], dim=0)
                logits_easy = net(input_easy_nomix)
                logits_easy_for_energy = net(input_easy_nomix)
            else:
                logits_easy_for_energy= []
            if has_hard:
                if unlabeled_trainloader is not None:
                    all_inputs = torch.cat([inputs_hard1, inputs_hard2, inputs_u, inputs_u2], dim=0)
                    all_targets = torch.cat([targets_hard, targets_hard, targets_u, targets_u], dim=0)
                else:
                    all_inputs = torch.cat([inputs_hard1, inputs_hard2], dim=0)
                    all_targets = torch.cat([targets_hard, targets_hard], dim=0)
                ...
                logits_mixed = net(mixed_input)
                # print("logits_mixed:",len(logits_mixed))
                len_hard = len(targets_hard) * 2
                # print("len_hard:",len_hard)
                logits_hard = logits_mixed[:len_hard]
                targets_hard_mix = mixed_target[:len_hard]

                if unlabeled_trainloader is not None:
                    logits_u = logits_mixed[len_hard:]
                    targets_u_mix = mixed_target[len_hard:]
                else:
                    logits_u = None
                    targets_u_mix = None
            else: 
                if unlabeled_trainloader is not None:
                    ...
                    mixed_target = l * target_a + (1 - l) * target_b 
                    logits_mixed = net(mixed_input)
                    logits_u = logits_mixed
                    targets_u_mix = mixed_target
                else:
                    continue
            # 로짓 결합
            if has_easy and has_hard:
                logits_x = torch.cat([logits_easy, logits_hard], dim=0)
                targets_x = torch.cat([target_easy_nomix, targets_hard_mix], dim=0)
                
            elif has_easy:
                logits_x = logits_easy
                targets_x = target_easy_nomix

            elif has_hard:
                logits_x = logits_hard
                targets_x = targets_hard_mix
            ...
            if len(logits_easy_for_energy) > 0:
                energy_loss_val = energy_loss(logits_easy_for_energy)
    ...
    def create_model(devices=[0]):
        model = ResNet18_64x64(num_classes=args.num_class)
        model = model.cuda()
        model = torch.nn.DataParallel(model, device_ids=devices).cuda()
        return model

    loader = dataloader.Tinyimagenet_dataloader(
    ...
    loader_easy = dataloader_easy_tinyimagenet2.easy_dataloader(
        dataset=args.dataset,
        r=args.r,
        noise_mode=args.noise_mode,
        batch_size=args.batch_size,
    ...
    if args.pretrained_path != "":
        with open(args.pretrained_path + f"/saved/{args.seed}/{args.preset}_energy0.01.pth.tar", "rb") as p:
    ...
    while epoch < args.num_epochs:
        ...
        saved_model = os.path.join(
            args.checkpoint_path, "saved", str(args.seed), f"{args.preset}_energy0.01.pth.tar"
        )
        torch.save(data_dict, saved_model)


        saved_model_300 = os.path.join(
        args.checkpoint_path, "saved", str(args.seed), f"{args.preset}_ep300_energy0.01.pth.tar")
