...
import dataloader_TIN as dataloader
from model import *
...
parser.add_argument('--noise_file', type=str, help='ImageNet200_txt', default=None)
...
def energy_loss(logits, labels):
    """
    logits: [batch_size, num_classes]
    labels: [batch_size] in one-hot or hard-labelif idx_chosen.numel() > 0:
    logits_net1 = outputs_x[idx_chosen]
    labels_net1 = pseudo_label_l[idx_chosen]
    loss_net1 += energy_loss(logits_net1, labels_net1, alpha=0.1) index form
    """
    energy = -torch.logsumexp(logits, dim=1)  # [batch_size]
    # If labels are one-hot, convert to index
    if labels.dim() == 2:
        labels = labels.argmax(dim=1)
    probs = F.softmax(logits, dim=1)
    correct_class_prob = probs[range(len(labels)), labels]
    return torch.mean(torch.abs(energy * (1 - correct_class_prob)))
...
def train(epoch, net, net2, optimizer, labeled_trainloader, pi1, pi2, pi1_unrel, pi2_unrel):
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, true_labels, index) in enumerate(labeled_trainloader):
        ...
        if idx_chosen.numel() > 0:
            logits_net1 = outputs_x[idx_chosen]
            labels_net1 = pseudo_label_l[idx_chosen]
            loss_net1 += 0.01*energy_loss(logits_net1, labels_net1)
        ...
        if idx_chosen_2.numel() > 0:
            ...
            loss_net2 += 0.01 * energy_loss(logits_net2, labels_net2)
...
def warmup(epoch, net, net2, optimizer, dataloader):
    ...
    # save_path = f"/home/yujin/ProMix2/save/seed{args.seed}/warmup_last_model_{args.dataset}_{args.noise_type}_{args.seed}_{args.rho_range}_{args.num_epochs}.pth.tar"
    # torch.save({'net1': net.state_dict(), 'net2': net2.state_dict()}, save_path)
...
def eval_train(model, all_loss, rho, num_class):
    ...
    targets_list = torch.zeros(100000)
    prediction_list = torch.zeros(100000)
    num_class = 200
...
def create_model():
    model = DualNet_TIN(args.num_class)
    model = model.cuda()
    return model
...
loader = dataloader.TIN_dataloader(args,log=stats_log)
...
for epoch in range(args.num_epochs + 1):
    ...
    print(f"[Epoch {epoch}] Estimated Time Remaining: {eta_formatted}")
