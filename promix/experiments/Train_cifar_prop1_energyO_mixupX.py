...
noise_type_map = {'clean': 'clean_label', 'worst': 'worse_label', 'aggre': 'aggre_label', 'rand1': 'random_label1',
                  ...
                  'noisy100': 'noisy_label','clean20':'clean_coarse_label','noisy20':'noisy_coarse_label'}
args.noise_type = noise_type_map[args.noise_type]

wandb.init(
    project="ProMix_Noisy_dataset",  # 프로젝트 이름 설정
    name=f"{args.dataset}_{args.noise_type}_{args.seed}_{args.rho_range}_{args.num_epochs}_{args.lr}",  
    config=args,  # args를 wandb에 기록
)
...
def train(epoch, net, net2, optimizer, labeled_trainloader, pi1, pi2, pi1_unrel, pi2_unrel):
    ...
    ################################################################
    # log_file_path = "train_loss_log.txt"
    # idx_file_path = "train_idx_log.txt"
    ################################################################
    ...
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, true_labels, index) in enumerate(labeled_trainloader):
        ...
        with torch.no_grad():
            ...
            # with open("pi1_unrel_log.txt", "a") as log3:
            #     log3.write("pi1_unrel:\n")
            #     log3.write(", ".join([f"{x:.6f}" for x in pi1_unrel.cpu().numpy()]) + "\n")
            # with open("pi2_unrel_log.txt", "a") as log4:
            #     log4.write("pi2_unrel:\n")
            #     log4.write(", ".join([f"{x:.6f}" for x in pi2_unrel.cpu().numpy()]) + "\n")
            ...
            ##################################################################################
            # with open(idx_file_path, "a") as idx_log:
            #     idx_log.write(f"[Epoch {epoch}, Batch {batch_idx}]\n")
            #     idx_log.write(f"idx_chosen: {idx_chosen.tolist()}\n")
            #     idx_log.write(f"idx_unchosen: {idx_unchosen.tolist()}\n")
            #     idx_log.write(f"idx_chosen_2: {idx_chosen_2.tolist()}\n")
            #     idx_log.write(f"idx_unchosen_2: {idx_unchosen_2.tolist()}\n\n")
            ##################################################################################
        ...
        if idx_chosen_2.numel() > 0:
            ...
            loss_cr2, loss_cr2_ph, loss_ce2, loss_ce2_ph = 0,0,0,0
        loss_net2 = loss_ce2 + w * (loss_cr2)# + loss_mix2 + loss_fmix2)
        if idx_chosen_2.numel() > 0:
            logits_net2 = outputs_a[idx_chosen_2]
            labels_net2 = pseudo_label_l2[idx_chosen_2]
            loss_net2 += 0.001 * energy_loss(logits_net2)
        ...
        ptx2 = ptx2 / (ptx2.sum(dim=1, keepdim=True)+1e-8)
        targets_urel2 = ptx2
        if idx_unchosen.numel() > 0:
        ...
        #     'epoch': epoch,
        #     'batch_idx': batch_idx,
        #     'loss_cr': float(loss_cr) if isinstance(loss_cr, torch.Tensor) else loss_cr,
        #     'loss_cr_ph': float(loss_cr_ph),
        #     'loss_ce': float(loss_ce),
        #     'loss_ce_ph': float(loss_ce_ph),
        #     'loss_unrel_ph': float(loss_unrel_ph),
        #     'loss_cr2': float(loss_cr2),
        #     'loss_cr2_ph': float(loss_cr2_ph),
        #     'loss_ce2': float(loss_ce2),
        #     'loss_ce2_ph': float(loss_ce2_ph),
        #     'loss_unrel2_ph': float(loss_unrel2_ph)
        # }
        # log_line = (
        #     f"[Epoch {loss_vals['epoch']}, Batch {loss_vals['batch_idx']}] "
        #     f"cr: {loss_vals['loss_cr']:.4f}, cr_ph: {loss_vals['loss_cr_ph']:.4f}, "
        #     f"ce: {loss_vals['loss_ce']:.4f}, ce_ph: {loss_vals['loss_ce_ph']:.4f}, "
        #     f"unrel_ph: {loss_vals['loss_unrel_ph']:.4f}, "
        #     f"cr2: {loss_vals['loss_cr2']:.4f}, cr2_ph: {loss_vals['loss_cr2_ph']:.4f}, "
        #     f"ce2: {loss_vals['loss_ce2']:.4f}, ce2_ph: {loss_vals['loss_ce2_ph']:.4f}, "
        #     f"unrel2_ph: {loss_vals['loss_unrel2_ph']:.4f}\n"
        # )
        # with open("train_loss_log.txt", "a") as log:
        #     log.write(log_line)
        #     log.flush()
        #################################################################################
...
def evaluate(loader, model, save = False, best_acc = 0.0):
    ...
    acc = 100*float(correct)/(float(total)+1e-8)
    if save:
        if acc > best_acc:
...
def eval_train(model, all_loss, rho, num_class):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, index) in enumerate(eval_loader):
            ...
            softmax_probs_list.append(softmax_probs.cpu().numpy())
            ###
            ...
            #수정
            loss_values_list.append(loss.cpu().numpy())
            ###
    ...
    #수정
    softmax_probs_list = np.concatenate(softmax_probs_list, axis=0)
    loss_values_list = np.concatenate(loss_values_list, axis=0)
    ###
    ...
    return prob, all_loss,softmax_probs_list,loss_values_list

class NegEntropy(object):
    def __call__(self, outputs):
        outputs = outputs.clamp(min=1e-12)
        probs = torch.softmax(outputs, dim=1)
        return torch.mean(torch.sum((probs+1e-8).log() * probs, dim=1))
...
for epoch in range(args.num_epochs + 1):
    ...
    prev_net1_weights = dualnet.net1.state_dict()
    prev_net2_weights = dualnet.net2.state_dict()
    if epoch < warm_up:
        ...
        prob1, all_loss[0], softmax_probs1, loss_values1 = eval_train(dualnet.net1, all_loss[0], rho, args.num_class)
        prob2, all_loss[0], softmax_probs2, loss_values2 = eval_train(dualnet.net2, all_loss[0], rho, args.num_class)
        
        pred1 = (prob1 > args.p_threshold)
        
        loss_save = [loss_values1,loss_values2]
        train_loss_1 = np.mean(loss_values1)
        train_loss_2 = np.mean(loss_values2)
        train_loss_mean = np.mean([train_loss_1, train_loss_2], axis=0)
        wandb.log({"epoch": epoch, "train_loss": train_loss_mean})
